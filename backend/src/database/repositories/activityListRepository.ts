...
class ActivityListRepository {
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    await ActivityList(options.database).updateOne(
      { _id: id },
      {
        ...data,
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await ActivityList(options.database).deleteOne({ _id: id }, options);
  ...
  static async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.activityDescription) {
        criteriaAnd.push({
          activityDescription: {
            $regex: MongooseQueryUtils.escapeRegExp(
              filter.activityDescription,
            ),
            $options: 'i',
          },
        });
    ...
    let rows = await ActivityList(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort);

    const count = await ActivityList(
      options.database,
  ...
  static async _createAuditLog(action, id, data, options: IRepositoryOptions) {
    await AuditLogRepository.log(
      {
        entityName: ActivityList(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
...
export default ActivityListRepository;
