...
class AssumptionLogRepository {
  ...
  static async update(id: string, data:any, options: IRepositoryOptions) {
    ...
    await AssumptionLog(options.database).updateOne(
      { _id: id },
      {
        ...data,
  ...
  static async destroy(id:string, options: IRepositoryOptions) {
    ...
    await AssumptionLog(options.database).deleteOne({ _id: id }, options);
  ...
  static async findById(id, options: IRepositoryOptions) {
    ...
    let record = await MongooseRepository.wrapWithSessionIfExists(
      AssumptionLog(options.database)
  ...
  static async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.assumptionLog) {
        criteriaAnd.push({
          assumptionLog: {
            $regex: MongooseQueryUtils.escapeRegExp(
              filter.assumptionLog,
            ),
            $options: 'i',
          },
        });
    ...
    let rows = await AssumptionLog(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort);

    const count = await AssumptionLog(
      options.database,
  ...
  static async findAllAutocomplete(search, limit, options: IRepositoryOptions) {
    ...
    const sort = MongooseQueryUtils.sort('assumptionLog_ASC');
    const limitEscaped = Number(limit || 0) || undefined;
  ...
  static async _createAuditLog(action, id, data, options: IRepositoryOptions) {
    await AuditLogRepository.log(
      {
        entityName: AssumptionLog(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
...
export default AssumptionLogRepository;
