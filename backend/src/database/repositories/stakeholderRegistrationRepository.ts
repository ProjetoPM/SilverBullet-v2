...
class StakeholderRegistrationRepository {
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    await StakeholderRegistration(options.database).updateOne(
      { _id: id },
      {
        ...data,
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await StakeholderRegistration(options.database).deleteOne({ _id: id }, options);
  ...
  static async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.mainRoleintheProject) {
        criteriaAnd.push({
          mainRoleintheProject: filter.mainRoleintheProject
        });
      }
      ...
      if (filter.positioninOrganization) {
        criteriaAnd.push({
          positioninOrganization: {
            $regex: MongooseQueryUtils.escapeRegExp(
              filter.positioninOrganization,
            ),
            $options: 'i',
          },
        });
      ...
      if (filter.mainProjectResponsibility) {
        criteriaAnd.push({
          mainProjectResponsibility: {
            $regex: MongooseQueryUtils.escapeRegExp(
              filter.mainProjectResponsibility,
            ),
            $options: 'i',
          },
        });
      ...
      if (filter.essentialRequirements) {
        criteriaAnd.push({
          essentialRequirements: {
            $regex: MongooseQueryUtils.escapeRegExp(
              filter.essentialRequirements,
            ),
            $options: 'i',
          },
        });
      ...
      if (filter.mainExpectations) {
        criteriaAnd.push({
          mainExpectations: {
            $regex: MongooseQueryUtils.escapeRegExp(
              filter.mainExpectations,
            ),
            $options: 'i',
          },
        });
      ...
      if (filter.phaseofGreaterInterest) {
        criteriaAnd.push({
          phaseofGreaterInterest: {
            $regex: MongooseQueryUtils.escapeRegExp(
              filter.phaseofGreaterInterest,
            ),
            $options: 'i',
          },
        });
    ...
    let rows = await StakeholderRegistration(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort);

    const count = await StakeholderRegistration(
      options.database,
  ...
  static async _createAuditLog(action, id, data, options: IRepositoryOptions) {
    await AuditLogRepository.log(
      {
        entityName: StakeholderRegistration(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
...
export default StakeholderRegistrationRepository;
